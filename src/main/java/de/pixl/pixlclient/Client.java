...
import de.pixl.pixlclient.input.keys.NameTagInF5Key;
...
public class Client {
    ...
    private static final ConcurrentLinkedDeque<UUID> players = new ConcurrentLinkedDeque<>();
    ...
    private static void init() {
        keyManager.register(NameTagInF5Key.getMapping(), new NameTagInF5Key());
    ...
    public static void tick() {
        ...
        if (connection != null && backend.isConnected()) {
            ConcurrentLinkedDeque<UUID> current = new ConcurrentLinkedDeque<>(connection.getOnlinePlayerIds());
