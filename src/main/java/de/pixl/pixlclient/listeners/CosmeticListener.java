...
public class CosmeticListener implements ListenerAdapter {
    ...
    @EventHandler
    public void handeBackendConnect(BackendReadyEvent event) {
        ...
        event.getBackend().send(JsonParser.parse("{}")
                ...
                .set("cape", (Client.capeManager.getCurrent() == null ? "noCapeSet" : Client.capeManager.getCurrent().name()))
                .set("data", uuids.stream().filter(uuid -> !uuids.contains(self)).map(UUID::toString).toList()));
    }

    @EventHandler
    public void handeBackendMessage(BackendMessageRecievedEvent event) {
